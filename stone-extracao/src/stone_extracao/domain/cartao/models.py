...
class TransacaoCartao(BaseModel):
    """Contrato de domínio da transação de cartão (publicado na fila)."""
...
class EventoFilaCartao(BaseModel):
    """Envelope da fila stone.cartao.transactions (contrato entre os dois serviços)."""

    event_type: str = "cartao.transaction"
    source: str = "stone.conciliation"
    received_at: datetime
    attempt: int = Field(default=1, ge=1, description="Tentativa atual de processamento")
